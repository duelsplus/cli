...
import { getConfig, setConfig, getConfigValue } from "@core/settings";
...
export async function handleSettings(
  ...
  // Unknown subcommand
  const message = `${error}Unknown subcommand: ${subcommand}. Use 'get', 'set', or no subcommand to view all.${reset}`;
  console.log(message);
  if (!interactive)
    process.exit(1);
}
